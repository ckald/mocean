...
module Jekyll
  ...
  class Image < Liquid::Tag
    ...
    def render(context)
      ...
      markup = /^(?:(?<url_only>url)\s+)?(?:(?<preset>[^\s.:\/]+)\s+)?(?:(?<lazy>lazy)\s+)?(?<image_src>[^\s]+\.[a-zA-Z0-9]{3,4})\s*(?<html_attr>[\s\S]+)?$/.match(render_markup)
      preset = settings['presets'][ markup[:preset] ]
      url_only = markup[:url_only]
      src_attr = if markup[:lazy]
        'data-src'
      else
        'src'
      end
      ...
      image_data = generate_image(instance, site.source, site.dest, settings['source'], settings['output'])
      generated_src = image_data[:path]
      ...
      if url_only
        ...
        return "<img #{src_attr}=\"#{site.baseurl}#{generated_src}\" data-width=#{image_width} data-height=#{image_height} #{html_attr_string}>"
      end
    end
    ...
    def generate_image(instance, site_source, site_dest, image_source, image_dest)
      ...
      # digest = Digest::MD5.hexdigest(image.to_blob.slice!(0..100)).slice!(0..5)
      ...
      gen_width = if instance[:width] && instance[:width] != "AUTO"
        instance[:width].to_f
      ...
      gen_height = if instance[:height] && instance[:height] != "AUTO"
      ...
      {
        :path => Pathname.new(File.join("/", image_dest, image_dir, gen_name)).cleanpath,
        :width => gen_width,
        :height => gen_height
      }
    end
